...
\subsubsection{Module D: Multi-Frame Refinement}

This optional module reduces measurement noise by aggregating estimates across multiple frames:

\begin{enumerate}
   \item The user captures a short video while keeping the target object in view.
   \item Selected measurement points are tracked across frames using Lucas-Kanade optical flow or feature matching (ORB/SIFT).
   \item For each frame $i$, the dynamic intrinsics $K_{out,i}$ are computed and the corresponding measurement module yields estimate $d_i$.
   \item Outliers are filtered using RANSAC or interquartile range (IQR).
   \item The final result is the median or mean of valid estimates:
         \begin{equation}
            d_{final} = \text{median}(d_1, d_2, \ldots, d_n)
         \end{equation}
\end{enumerate}
...
\begin{figure}[htbp]
   ...
   \begin{tikzpicture}[
      ...
      \node[module, below=1.5cm of select, xshift=-4cm, fill=teal!15] (modA) {Module A\\Ground-Plane};
      \node[module, below=1.5cm of select, xshift=-1.3cm, fill=cyan!15] (modB) {Module B\\Planar Object};
      \node[module, below=1.5cm of select, xshift=1.3cm, fill=orange!15] (modC) {Module C\\Single-View};
      \node[module, below=1.5cm of select, xshift=4cm, fill=gray!15] (modD) {Module D\\Multi-Frame};
      ...
      \draw[arrow] (select.south) -- ++(0,-0.4) -| (modD.north);

      % Arrows from modules to result
      \draw[arrow] (modA.south) -- ++(0,-0.3) -| (result.north west);
      \draw[arrow] (modB.south) -- ++(0,-0.3) -| ([xshift=-0.5cm]result.north);
      \draw[arrow] (modC.south) -- ++(0,-0.3) -| ([xshift=0.5cm]result.north);
      \draw[arrow] (modD.south) -- ++(0,-0.3) -| (result.north east);
...
\begin{itemize}
   ...
   \item \textbf{Processing Options}:
         \begin{itemize}
            ...
            \item Toggle multi-frame averaging
         \end{itemize}
...
\subsubsection{Multi-Frame Averaging}

Aggregating measurements across multiple frames is expected to reduce random errors from:
\begin{itemize}
   \item Hand shake and camera motion
   \item IMU noise and orientation fluctuations
   \item Point selection variability
\end{itemize}
